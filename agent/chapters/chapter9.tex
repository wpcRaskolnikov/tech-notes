\chapter{检索增强生成(RAG)}
\section{模型微调}
在前面的章节中，我们了解到大型语言模型（LLM）的强大能力。然而，LLM 也存在一些固有的局限性：
\begin{itemize}
...
常见的微调方式有：
\begin{itemize}
	\item \textbf{全参数微调}：更新模型的所有参数，效果最好但成本最高
	\item \textbf{LoRA}：冻结原始参数，只训练低秩适配矩阵，大幅降低显存需求
	\item \textbf{Prompt Tuning}：只学习少量提示向量，参数效率最高
\end{itemize}

微调的优点是模型将知识内化到参数中，推理时不需要额外的检索步骤，响应速度更快。但微调也有明显的缺点：需要高质量的训练数据，训练成本较高，而且一旦知识更新就需要重新训练。
相比之下，RAG 不需要修改模型参数，只需维护外部知识库，知识更新更灵活。实际应用中，微调和 RAG 并不矛盾，很多系统会同时使用两者来获得最佳效果。

\section{RAG}
检索增强生成（Retrieval-Augmented Generation，RAG）是一种结合检索和生成的技术。它的核心思想是：在生成回答之前，先从外部知识库中检索相关信息，然后将检索到的信息作为上下文提供给 LLM，让模型基于这些真实的信息生成回答。
...
传统数据库就像电话簿，你通过名字（精确匹配）查找电话号码。向量数据库更像是一个智能地图，你说"我想找个附近好吃的中餐馆"，它会返回地理位置相近的选项。
常见的向量数据库包括：FAISS、Pinecone、Milvus和ChromaDB。
向量数据库的核心功能是近似最近邻搜索（Approximate Nearest Neighbor, ANN）。当查询向量到来时，数据库快速找到与之最相似的 $k$ 个向量。

\section{RAG 的工作流程}

让我们通过一个完整的例子来理解 RAG 的工作流程，假设你有一个公司内部的技术文档库，员工可以提问技术问题。
